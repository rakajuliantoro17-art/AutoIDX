...
Live Trading Fase Canary (Testing Skala Kecil)
Gerbang TAMBAHAN khusus fase testing live trading dengan nominal
kecil (`services/liveTrading/risk/liveTradingConfig.ts` - sebelumnya
orphan, diaktifkan sesi audit). Ini terpisah dari `BOT_MODE`/
`BOT_LIVE_CONFIRM` yang sudah ada - kalau salah satu env var wajib
di bawah tidak diisi, defaultnya FAIL-CLOSED (canary tetap aktif /
BUY live diblokir), bukan diam-diam full live tanpa batas.
PENTING: kalau kamu sudah pernah menjalankan live trading
sebelum fitur ini di-deploy, `BOT_CANARY_ENABLED` defaultnya FALSE
artinya BUY live akan mulai ditolak begitu ini di-deploy, SAMPAI
kamu set env var ini ke `"true"` di Vercel. Ini disengaja (extra
lapis aman), bukan bug.
Variabel	Wajib	Keterangan
`BOT_CANARY_ENABLED`	Ya (untuk live BUY)	Harus persis `"true"` - default FALSE, BUY live diblokir kalau belum diisi
`BOT_CANARY_ONLY`	Tidak	Default `true` (aktif) kecuali di-set `"false"` eksplisit - saat aktif, nominal per trade dibatasi `BOT_CANARY_MAX_TRADE_AMOUNT`
`BOT_CANARY_MAX_TRADE_AMOUNT`	Tidak	Default 25.000 (IDR) - batas nominal per trade SELAMA fase canary. BEDA dari `BOT_MAX_TRADE_AMOUNT` (itu batas umum di `config/bot.ts`, tidak berhubungan)
