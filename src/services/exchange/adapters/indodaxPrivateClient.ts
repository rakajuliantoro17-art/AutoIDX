...
Memakai RequestSigner generik yang SUDAH ADA di
services/exchange/utils/signer.ts — file ini hanya menyusun
`totalParams` (query string) spesifik Indodax, signing-nya
didelegasikan, bukan diimplementasikan ulang.
...
==========================================================
*/
import { RequestSigner } from "../utils";
...
export class IndodaxPrivateClient {
  ...
  async call<T>(
    ...
    const totalParams = this.buildTotalParams(requestParams);
    const signature = RequestSigner.sha512(
      totalParams,
