...
export class MarketScanner {
  async scanMarket(
    ...
    // FIX (503 di Vercel Hobby / drift threshold): sebelumnya default
    // di sini hardcode 50_000_000, TERPISAH dari TRADING_CONFIG.minVolumeIdr
    // di config/trading.ts -- scanCycle.ts (dipanggil cron) memanggil
    // scanMarket() TANPA argumen sama sekali, jadi selalu jatuh ke
    // hardcode di sini, BUKAN nilai yang sudah di-tune di config.
    // Sekarang keduanya satu sumber, supaya menaikkan/menurunkan
    // threshold di config/trading.ts (atau env var BOT_MIN_VOLUME_IDR)
    // benar-benar berlaku untuk SEMUA jalur scan, termasuk cron.
    const minVolume = criteria.minVolumeIdr ?? TRADING_CONFIG.minVolumeIdr;
